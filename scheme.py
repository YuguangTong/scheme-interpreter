...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            return self.fn(*args_in_py), None
        except TypeError:
            raise SchemeError
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        check_formals(formals)
        parameter = Pair(formals, vals.second)
        lambda_fn = do_lambda_form(parameter, env)[0]
        env.define(fn_name, lambda_fn)
        return fn_name, None
...
def check_formals(formals):
    ...
    for elem in formals:
        if not scheme_symbolp(elem):
            raise SchemeError("{0} is not a symbol".format(str(elem)))
    formals_in_py_lst = [formals[i] for i in range(n)]
    if n != len(set(formals_in_py_lst)): 
        raise SchemeError("contain repeated symbols:" + str(formals))
